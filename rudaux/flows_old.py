...
async def run(args):
    ...
    await start_prefect_agent(work_queue=settings.prefect_queue_name, work_queues=[settings.prefect_queue_name])
    os.system(f"prefect agent start --work-queue {settings.prefect_queue_name}")
    # print('starting work_queues: ', [settings.prefect_queue_name])
    # orion_agent = OrionAgent(work_queues=[settings.prefect_queue_name])
    # await orion_agent.start()
    # submittable_runs = await orion_agent.get_and_submit_flow_runs()
    # print(submittable_runs)
