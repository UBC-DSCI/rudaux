...
def get_groups(api_info, course_id):
    ...
    return [{
             'name' : g['name'],
             'lms_id' : str(g['id']),
             'members' : [str(m['user_id']) for m in _get(api_info[course_id], str(g['id'])+'/memberships', use_group_base=True)]
            } for g in grps]

def get_assignments(api_info, course_id):
    ...
    logger = get_logger()
    logger.info(f"Retrieved {len(processed_asgns)} assignments from LMS for {config.course_ids[course_id]}")
...
def get_submissions(api_info, course_id, assignment):
    ...
    logger = get_logger()
    logger.info(f"Retrieved {len(processed_subms)} submissions for assignment {assignment['name']} from LMS for {course_id}")
...
def update_grade(api_info, course_id, submission):
    # post the grade
    _put(api_info[course_id], 'assignments/'+assignment['id']+'/submissions/'+student['id'], {'submission' : {'posted_grade' : score}})

    # check that it was posted properly
    canvas_grade = str(_get(api_info[course_id], 'assignments/'+assignment['id']+'/submissions/'+student['id'])[0]['score'])
