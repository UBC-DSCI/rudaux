            ...
            if asgn.due_at < plm.now():
                ...
                if (n_total - n_outstanding)/n_total >= self.config.return_solution_threshold: 
                    ...
                    if plm.now() > plm.parse(self.config.earliest_solution_return_date, tz=self.course_info['time_zone']):
                        retsoln_results = self.process(Submission.return_solution, submissions, {key : val for (key, val) in prep_results.items() if posted_grades[key]}, [SubmissionStatus.MISSING, SubmissionStatus.PREPARED])
                    else:
