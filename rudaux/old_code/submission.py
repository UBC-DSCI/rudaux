...
def build_submission_set(config, subm_set):
    ...
    for course_name in subm_set:
        ...
        all_posted = all_posted and all(
            [subm['posted_at'] is not None for subm in subm_set[course_name]['submissions']])
        for subm in subm_set[course_name]['submissions']:
            ...
            if os.path.exists(subm['student_folder']):
                ...
                all_returned = all_returned and (os.path.exists(subm['fdbk_path']) or (
                            subm['score'] == 0 and not os.path.exists(subm['snapped_assignment_path'])))
    if all_posted and all_returned:
        raise signals.SKIP(
            f"All grades are posted, all solutions returned, and all feedback returned for assignment {subm_set['__name__']}. Workflow done. Skipping.")
            ...
            if subm['due_at'] < plm.now():
                if not os.path.exists(subm['soln_path']):
                    ...
                    if os.path.exists(subm['student_folder']):
                        ...
                        logger.warning(
                            f"Warning: student folder {subm['student_folder']} doesnt exist. Skipping solution return.")
            # else:
        ...
        for subm in subm_set[course_name]['submissions']:
            if subm['status'] == GradingStatus.COLLECTED:
                ...
                try:
                    ...
                    msg = (
                                f"JSON ERROR in student {student['name']} {student['id']} assignment {assignment['name']} grader {grader['name']} course name {course_name}" +
...
def await_completion(subm_set):
    ...
    for course_name in subm_set:
        ...
        all_done = all_done and all(
            [subm['status'] != GradingStatus.NEEDS_MANUAL_GRADE for subm in subm_set[course_name]['submissions']])
    if not all_done:
        raise signals.SKIP(
            f"Submission set {subm_set['__name__']} not done grading yet. Skipping uploading grades / returning feedback")
    return subm_set
