...
# 
# 4. Add `rudaux` to the PATH for root and sudo user. To do this become the super user via `sudo su -` and then in HOME add `rudaux` to the PATH in `.bash_profile` via `export PATH="/usr/local/bin:$PATH"`, and then edit `/etc/sudoers` so that `/usr/local/bin/` is added to the  `secure_path` (it should read: `Defaults    secure_path = /sbin:/bin:/usr/sbin:/usr/bin:/usr/local/bin`).
# 
